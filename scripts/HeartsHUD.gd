...
var full_color := Color(0.89, 0.23, 0.23, 1)
...
func _draw() -> void:
	...
	var half := (int(player.hp) % int(hp_per_heart)) >= int(hp_per_heart * 0.5)
	for i in range(total_hearts):
		var origin := Vector2(i * heart_spacing, 0.0)
		var col := empty_color
		if i < full_hearts:
			col = full_color
		elif i == full_hearts and half:
			col = full_color
		_draw_heart(origin, heart_size, col)
